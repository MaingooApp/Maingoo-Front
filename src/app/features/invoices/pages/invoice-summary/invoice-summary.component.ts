...
export class InvoiceSummaryComponent implements OnInit {
  ...
  onUpload(event: any) {
    ...
    if (
      !fileType ||
      (!validTypes.includes(fileType) && !file.name.toLowerCase().match(/\.(jpg|jpeg|png|webp|heic|heif|pdf)$/))
    ) {
  ...
  private onAnalysisFailed(document: AnalysisDocument) {
    ...
    this.toastService.error(
      'Análisis fallido',
      'No se pudo analizar el documento. Por favor verifica que sea una imagen clara de una factura.',
      5000
    );
  }
