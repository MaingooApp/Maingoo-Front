...
export class DocGeneratorComponent {
  ...
  get filteredDocuments(): DocumentCard[] {
    ...
    if (this.documentSearchQuery.trim()) {
      ...
      filtered = filtered.filter(
        (doc) => doc.title.toLowerCase().includes(query) || doc.tags.some((tag) => tag.toLowerCase().includes(query))
      );
    }
  ...
  saveCameraModal() {
    ...
    this.toastService.success(
      'Cámara Añadida',
      `${camera.name} (${camera.type === 'positive' ? 'Frigorífico' : 'Congelador'}) añadida correctamente`
    );
  ...
  saveTemperatureRegistration() {
    ...
    this.toastService.success(
      ...
      `${this.newTemperature}°C registrados para ${this.selectedCamera.name}`
    );
