...
import { KpiSlotVM, KpiSlideVM, KpiSlotClickEvent, KpiSlideChangeEvent } from '../../interfaces/kpi-slot.interfaces';
...
@Component({
  ...
  template: `
    ...
    <div
      class="mg-surface group rounded-content shadow-sm overflow-hidden 
      ...
      <!-- Header del slot -->
      <div
        class="px-4 py-3 border-b border-surface flex items-center justify-between bg-surface-50/50 dark:bg-surface-800/50">
        <div class="flex items-center gap-2">
          <h3 class="text-sm font-semibold mg-text m-0">{{ slot?.title || 'Cargando...' }}</h3>
          <!-- Badge indicador de datos mock -->
          <span *ngIf="slot?.isMock" class="mg-demo-badge">False data | No Backend</span>
        </div>
        ...
        <div *ngIf="visibleSlides().length > 1" class="flex items-center gap-1.5">
          <button
            *ngFor="let slide of visibleSlides(); let i = index"
            ...
            class="w-1.5 h-1.5 rounded-full transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-1 focus:ring-offset-surface-0 dark:focus:ring-offset-surface-900"
            [ngClass]="
              currentIndex() === i
                ? 'bg-primary w-3'
      ...
      <div *ngIf="slot?.error && !slot?.loading" class="p-4">
        ...
        <button
          ...
          (click)="onRetryClick()"></button>
      </div>

      <!-- Estado: Empty -->
      <div *ngIf="!slot?.loading && !slot?.error && visibleSlides().length === 0" class="p-4">
        <p class="mg-text-muted text-sm text-center m-0">Sin datos disponibles</p>
      </div>

      <!-- Estado: OK - Contenido de slides -->
      <div *ngIf="!slot?.loading && !slot?.error && visibleSlides().length > 0" class="relative overflow-hidden">
        <!-- Slide actual -->
        <div
          *ngIf="currentSlide() as slide"
          ...
          class="w-full p-4 text-left bg-surface-0 dark:bg-surface-900">
          <!-- Contenido de la slide -->
          <div class="flex items-start justify-between gap-3">
            <div class="flex-1 min-w-0">
              <!-- Valor principal -->
              <div class="flex items-baseline gap-1 mb-1">
                <span class="text-2xl font-bold" [ngClass]="getValueClasses(slide.severity)">
                  {{ formatValue(slide.value, slide.valueFormatter) }}
                </span>
                <span *ngIf="slide.suffix" class="text-sm mg-text-muted font-medium">
                  {{ slide.suffix }}
                </span>
              </div>

              <!-- Label -->
              <p class="text-xs mg-text-muted m-0 truncate">{{ slide.label }}</p>
            </div>
          ...
          <div
            ...
            [class.opacity-0]="!slide.severity || slide.severity === 'neutral'"></div>
        </div>
        ...
        <div
          *ngIf="visibleSlides().length > 1"
          ...
          <button
            ...
            class="pointer-events-auto w-6 h-6 rounded-full bg-surface-0/80 hover:bg-surface-0 dark:bg-surface-800/80 dark:hover:bg-surface-800 
                   shadow-sm border border-surface flex items-center justify-center
                   mg-icon-muted hover:text-primary transition-all
                   focus:outline-none focus:ring-2 focus:ring-primary opacity-0 group-hover:opacity-100">
...
export class KpiSlotComponent implements OnInit, OnDestroy, OnChanges {
  ...
  private initializeSlot(): void {
    ...
    const priorityIndex = slides.findIndex((s) => s.severity === 'danger' || s.severity === 'warn');
  ...
  prevSlide(reason: 'auto' | 'manual' = 'manual'): void {
    const slides = this.visibleSlides();
    const newIndex = this.currentIndex() === 0 ? slides.length - 1 : this.currentIndex() - 1;
  ...
  getValueClasses(severity?: string): string {
    switch (severity) {
      case 'ok':
        return 'text-green-600 dark:text-green-300 group-hover:text-green-700 dark:group-hover:text-green-200';
      case 'warn':
        return 'text-amber-600 dark:text-amber-300 group-hover:text-amber-700 dark:group-hover:text-amber-200';
      case 'danger':
        return 'text-red-600 dark:text-red-300 group-hover:text-red-700 dark:group-hover:text-red-200';
      default:
        return 'mg-text group-hover:text-primary';
    }
  }
