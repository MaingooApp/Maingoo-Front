...
export class SupplierDeliveryComponent implements OnChanges {
  ...
  ngOnChanges(changes: SimpleChanges) {
    if (changes['supplier'] && this.supplier) {
      ...
      this.selectedLastOrderDays = this.supplier.orderDays
        ? this.supplier.orderDays.split(',').map((d) => d.trim())
        : [];
    }
  }
  ...
  get hasEmptyDeliveryFields(): boolean {
    ...
    const minPriceEmpty = this.supplier.minPriceDelivery === null || this.supplier.minPriceDelivery === undefined;
